...
public class Statistics {
   ...
   void getStat(String keyword) throws ClassNotFoundException, SQLException {
       ...
       while(rs.next()) {
		...
		//liệt kê các host
		sql = "SELECT DISTINCT host FROM articles WHERE user_id = ? AND id IN (SELECT websiteId FROM keyword_web WHERE keywordId = " + keywordId + ")";
		PreparedStatement statement1 = connection.prepareStatement(sql);
		statement1.setInt(1, AppRun.userID);
		ResultSet rs1 = statement1.executeQuery();
		
	       while (rs1.next()) {
			String h = rs1.getString("host");
			sql = "SELECT * FROM articles WHERE user_id = ? AND host LIKE ? AND id IN (SELECT websiteId FROM keyword_web where keywordId = " + keywordId + ")";
			PreparedStatement statement2 = connection.prepareStatement(sql);
			statement2.setInt(1, AppRun.userID);
			statement2.setString(2, h);
		...
		String newsql = "SELECT DISTINCT date FROM articles WHERE user_id = ? AND id IN (SELECT websiteId FROM keyword_web WHERE keywordId = " + keywordId + ")";
		
		PreparedStatement statement3 = connection.prepareStatement(newsql);
		statement3.setInt(1, AppRun.userID);
		ResultSet rs3 = statement3.executeQuery();
		while (rs3.next()) {
			Date t = rs3.getDate("date");
			
			newsql = "SELECT * FROM articles WHERE user_id = ? AND date LIKE ? AND id IN (SELECT websiteId FROM keyword_web WHERE keywordId = " + keywordId + ")";
			PreparedStatement statement4 = connection.prepareStatement(newsql);
			statement4.setDate(2, t);
			statement4.setInt(1, AppRun.userID);
